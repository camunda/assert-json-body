...
function loadConfigOnce(configPath?: string) {
  if (!cachedConfig) {
    cachedConfig = buildConfig(configPath);
  }
  return cachedConfig;
}
...
function resolveResponsesFile(opts: ResolverOptions = {}): ResolvedPath {
  ...
  if (opts.responsesFilePath) {
    const p = isAbsolute(opts.responsesFilePath)
      ? opts.responsesFilePath
      : resolve(process.cwd(), opts.responsesFilePath);
  ...
  const configured =
    cfg.resolved.extract.responsesFile ||
    (cfg.resolved.extract.outputDir
      ? resolve(process.cwd(), cfg.resolved.extract.outputDir, 'responses.json')
      : undefined);
  if (configured) return { path: configured, source: 'config' };
  // 4. default (cwd/json-body-assertions/responses.json)
  return {
    path: resolve(process.cwd(), 'json-body-assertions', 'responses.json'),
    source: 'default',
  };
}
...
function buildIndex(filePath: string): Map<string, ResponseEntry[]> {
  ...
  try {
    ...
    throw new Error(`Failed to read responses schema at ${filePath}: ${(e as Error).message}`);
  }
  let parsed: ResponsesFile;
  try {
    parsed = JSON.parse(raw) as ResponsesFile;
  } catch (e) {
    throw new Error(
      `Failed to parse responses schema JSON at ${filePath}: ${(e as Error).message}`
    );
  }
...
function getIndex(filePath: string): Map<string, ResponseEntry[]> {
  ...
  if (!idx) {
    idx = buildIndex(filePath);
    responseIndexCache.set(filePath, idx);
  }
  return idx;
}

export interface PickRouteOptions extends ResolverOptions {
  method?: string;
  status?: string;
}
...
export function pickRoute(
  ...
  if (a && typeof a === 'object' && !Array.isArray(a)) {
    ...
    status = b;
    if (_d !== undefined)
      throw new Error('Fourth argument to pickRoute removed; use options object instead.');
  }
  const resolved = resolveResponsesFile({
    responsesFilePath: opts.responsesFilePath,
  ...
  if (entries.length === 0) {
    throw new Error(
      `No OpenAPI response spec entries found for path '${path}' (schema source: ${resolved.source} @ ${resolved.path}).`
    );
  }
  ...
  if ((method || status) && filtered.length === 0) {
    const available = entries
      ...
      .join(', ');
    throw new Error(
      `No matching spec entry for ${path} with method=${method || '*'} status=${status || '*'} (schema: ${resolved.path}).\nAvailable: ${available}`
    );
  }
  const chosen =
