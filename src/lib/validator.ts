...
const shouldTreatAsObject = (spec: FieldSpec): boolean => {
  if (spec.children) return true;
  if (spec.type === 'object') return true;
  if (/^[A-Z][A-Za-z0-9]+$/.test(spec.type) && !shouldTreatAsEnum(spec.type)) return true;
  return false;
};
const jsonPointerEscape = (seg: string) => seg.replace(/~/g, '~0').replace(/\//g, '~1');
const toExpectedDescriptor = (spec: FieldSpec): string => {
...
function _validateRouteContext(routeCtx: RouteContext, body: unknown) {
  ...
  const validateGroup = (
    ...
    for (const spec of specs) {
      ...
      if (!exists) {
        if (mode === 'required') {
          errors.push(`[MISSING] ${fieldPath} expected ${toExpectedDescriptor(spec)}`);
        }
        continue;
      }
      ...
      if (value === null) {
        if (!spec.nullable) {
          errors.push(`[TYPE] ${fieldPath} expected ${toExpectedDescriptor(spec)} but got null`);
        }
        continue;
      }
      const inner = extractArrayInner(spec.type);
      ...
      if (shouldTreatAsObject(spec)) {
        if (typeof value !== 'object' || Array.isArray(value)) {
          errors.push(`[TYPE] ${fieldPath} expected object but got ${actualDescriptor(value)}`);
          continue;
        }
        if (spec.children) {
          validateGroup(
 ...
 */
export interface ValidateResultBase {
  ok: boolean;
  errors?: string[];
  response: unknown;
...
export function validateResponseShape(
  ...
  if (doRecord) {
    ...
    const recordDir = resolveRecordDirectory({
      outputDir: cfg.resolved.extract.outputDir,
      configPath: options.configPath,
    });
  ...
  try {
    ...
    return {
      ok: false,
      errors: lines.slice(1).filter(Boolean),
      response: body,
