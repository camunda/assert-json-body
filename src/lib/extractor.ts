...
function resolveResponse(
  resp: OpenAPIV3.ResponseObject | OpenAPIV3.ReferenceObject,
  doc: ComponentsLike
...
export function extractResponses(doc: OpenAPIV3.Document): ResponseEntry[] {
  ...
  for (const [path, pathItem] of Object.entries(paths)) {
    for (const method of ['get', 'post', 'put', 'patch', 'delete'] as const) {
      const op: OperationLike | undefined = (pathItem as Record<string, unknown>)[method] as
        | OperationLike
        | undefined;
      if (!op) continue;
      ...
      for (const [status, response] of Object.entries(responses)) {
        ...
        const schema = appJson?.schema as
...
export async function generate(cfg?: ExtractConfig, options?: { configPath?: string }) {
  ...
  try {
    ...
    const targetFile = effective.responsesFile
      ? resolve(effective.responsesFile)
      : join(outDir, 'responses.json');
...
"Typed validator resolved to itself. Avoid aliasing the package name to the generated typed file. Use a relative import to the generated ./json-body-assertions/index or import from 'assert-json-body/base' for the core API."
        ...
        if (effective.logLevel === 'debug')
          console.warn('Failed to emit typed index.ts:', (e as Error).message);
      }
    }
    if (effective.logLevel !== 'silent') {
      console.log(
        `Extracted ${responses.length} response schemas from commit ${commit}` +
          (effective.dryRun ? ' (dry-run: no files written)' : '')
      );
      ...
      if (resolution.warnings.length) {
        for (const w of resolution.warnings) console.warn(`[config] ${w}`);
      }
    }
  } finally {
    if (workdir) {
