...
const components = {
  ...
  WrappedId: {
    allOf: [{ $ref: '#/components/schemas/SimpleString' }, { minLength: 36, maxLength: 36 }],
  },
  Composite: {
    allOf: [
      { properties: { a: { type: 'string' } }, required: ['a'] },
      { properties: { b: { $ref: '#/components/schemas/WrappedId' } } },
    ],
  },
  Node: {
    allOf: [
...
describe('describeType', () => {
  it('resolves primitive wrapper to primitive', () => {
    const t = describeType(
      { $ref: '#/components/schemas/WrappedId' },
